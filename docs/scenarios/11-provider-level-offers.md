...
|---|----------|--------|----------|
| SC-PO-01 | Publish catalog with resources + 1 resource-level offer + 1 provider-level offer (no resourceIds) | POST `/catalog/push` | HTTP 200 ACK |
...
| # | Scenario | Method | Expected |
|---|----------|--------|----------|
| SC-PO-06 | Offer with `resourceIds: []` (empty array) classified as provider-level | POST `/catalog/push` with offer having `"resourceIds": []` | HTTP 200 ACK |
...
| # | Scenario | Method | Expected |
|---|----------|--------|----------|
| SC-PO-09 | Discover API returns provider-level offers alongside resource-level offers | POST `/discover` with query matching R1/R2 | Response catalog's `offers` array includes BOTH `offer-item-${TS}` (resource-level) AND `offer-prov-${TS}` (provider-level) |
| SC-PO-10 | Provider-level offer attached to ALL catalogs from that provider | POST `/discover` | Every catalog with `provider.id = ${PROV_ID}` has `offer-prov-${TS}` in its `offers` array |
| SC-PO-11 | Provider-level offers NOT present in catalogs from OTHER providers | POST `/discover` returning catalogs from different providers | Catalogs from other providers do NOT contain `offer-prov-${TS}` |
...
curl -s http://localhost:8082/discover \
...
1. HTTP response: nested `{"message":{"status":"ACK","messageId":"..."}}` (HTTP 200)
