...
|---|----------|--------|----------|
| SC-26 | Push offer-only catalog (0 resources, 2 offers referencing R1 and R2) from different BPP | POST push | HTTP 200 ACK |
| SC-27 | No stub rows created | `SELECT count(*) FROM item WHERE id = 'R1-${TS}' AND catalog_id = '${CAT_ID}'` | Exactly 1 row (not 2) |
| SC-28 | Offer attached to existing item | `SELECT payload FROM item WHERE id = 'R1-${TS}' AND catalog_id = '${CAT_ID}'` | Payload JSON contains the cross-BPP offer |
| SC-29 | BPP identity preserved | `SELECT payload FROM item WHERE id = 'R1-${TS}' AND catalog_id = '${CAT_ID}'` | BPP identity inside `payload` JSON = original publisher BPP (NOT offer publisher); there is no `bpp_id` column |
