...
- [x] **`statement_timeout` on the legacy-fallback Postgres probe.** *(Implemented: `discovery.filter-grammar.probe-statement-timeout-ms`, default 2000ms, scoped to the single probe statement via `set_config(..., true)`. Verified via JDBC-call mock; the timeout actually firing under load requires a live Postgres connection, consistent with this package's existing Docker-only verification boundary.)*
...
3. **Statement timeout on the legacy probe.** New `discovery.filter-grammar.probe-statement-timeout-ms` (`DiscoveryProperties.FilterGrammar.probeStatementTimeoutMs`, default `2000`ms — tight, since this is a trivial `CAST` syntax probe, not a real query). Implemented as a single combined statement — `SELECT set_config('statement_timeout', :timeoutMs, true), CAST(:expression AS jsonpath)` — so `set_config(..., true)`'s "is_local" scoping applies for exactly this one implicit-autocommit-transaction statement and can never leak onto the pooled connection's next unrelated use (a bare two-statement `SET LOCAL` + probe would not compose correctly outside an explicit transaction block, which this job has no existing infrastructure for). Verified via a `JdbcClient` mock asserting the SQL text and the bound `timeoutMs` parameter; the timeout actually firing under real load needs a live Postgres connection and is out of this pass's no-Docker unit-test scope (consistent with how this package's other Postgres-execution behavior is already only verified at the Testcontainers-integration level).
