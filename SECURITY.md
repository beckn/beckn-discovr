...
Beckn Discovr is a catalog discovery → query → dispatch pipeline for the Beckn ecosystem (catalog-discover-job, catalog-publish-job, response-dispatcher). This document describes how to report security vulnerabilities found in this repository.

## Supported Versions

Only the latest published release is supported for security fixes. We ship frequently, so fixes go into the next release rather than being backported to older release lines.

If you're not on the latest tag, please upgrade before or shortly after filing a report so the fix reaches you.

## Reporting a Vulnerability

Report vulnerabilities privately using [GitHub Private Vulnerability Reporting](https://github.com/beckn/beckn-discovr/security/advisories/new):
...
- Affected job/service (catalog-discover-job, catalog-publish-job, response-dispatcher) and file
...
This policy covers the source code, build tooling, and documentation in the [beckn-discovr](https://github.com/beckn/beckn-discovr) repository.

Out of scope:

- The Beckn Protocol specification and other Beckn Foundation repositories
- Any networks and their participants running Beckn Discovr in production
- Downstream forks, integrations, or vendor distributions
- Third-party infrastructure not operated by this project (Elasticsearch, Kafka, PostgreSQL clusters you deploy yourself)

Reports about a live deployment you don't operate should go to that deployment's operator, not this repository.

## Good Faith Research

We support good-faith security research against this repository's source code — static analysis, local testing, and review of the code and example configs shipped here. We will not pursue legal action against researchers who:
