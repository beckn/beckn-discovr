...
**Beckn Protocol v2.0 (v2.1 field naming)** — context fields are camelCase (`transactionId`, `messageId`, `bapId`, `bapUri`); uses `resources` (not `items`), `resourceAttributes` (not `itemAttributes`); `@context`/`@type` only on Attributes objects; ACK = `{"status":"ACK"}`.
