...
- **Design agent**: after a design decision is approved, offer to record it as an ADR so the rationale is preserved alongside the code
- **Review agent**: flag PRs that introduce architectural changes without a corresponding ADR; suggest creating one before merge